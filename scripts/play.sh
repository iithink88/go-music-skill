...
top = ranked[0]
default_name = f"{slug(top.get('artist','unknown'))}-{slug(top.get('name','music'))}.mp3"
...
if os.path.exists(default_path) and os.path.getsize(default_path) > 1024:
    ...
    print(json.dumps({'cache_hit': True, 'path': out_path, 'size': os.path.getsize(out_path), 'reason': 'song_file_exists', 'chosen': top}, ensure_ascii=False))
    raise SystemExit(0)

os.makedirs(os.path.dirname(out_path), exist_ok=True)

# Download with multi-candidate / multi-source fallback: a source may return a
# dead stream (HTTP 404). Try the top scored candidates and use the first that
# actually downloads.
chosen = None
_tmp = out_path + '.part'
for _cand in ranked[:6]:
    _params = urllib.parse.urlencode({'id': _cand['id'], 'source': _cand['source']})
    _url = f'http://localhost:{port}/api/v1/music/stream?{_params}'
    _req = urllib.request.Request(_url, headers={'User-Agent': 'Mozilla/5.0'})
    try:
        with urllib.request.urlopen(_req) as resp, open(_tmp, 'wb') as f:
            while True:
                chunk = resp.read(65536)
                if not chunk:
                    break
                f.write(chunk)
        if os.path.getsize(_tmp) < 1024:
            raise Exception('downloaded file too small')
        chosen = _cand
        if _cand is not top:
            print(json.dumps({'source_fallback': True, 'preferred_source': top.get('source'),
                              'used_source': _cand.get('source'), 'name': _cand.get('name'),
                              'artist': _cand.get('artist')}, ensure_ascii=False))
        break
    except Exception as e:
        if os.path.exists(_tmp):
            os.remove(_tmp)
        print(json.dumps({'stream_failed': {'source': _cand.get('source'), 'name': _cand.get('name'),
                                            'artist': _cand.get('artist')}, 'reason': str(e)}, ensure_ascii=False))
        continue
...
# FIX (2026-08-04): the stream API can return M4A/AAC (e.g. kuwo) even when the
# caller asks for ".mp3". Detect the real container and transcode to a standard
# MP3 with ffmpeg when available, so downstream mutagen ID3 embedding works.
def _detect(path):
...
_fmt = _detect(out_path)
if _fmt != 'mp3':
    _ff = shutil.which('ffmpeg') or ''
    _ext_map = {'m4a': '.m4a', 'ogg': '.ogg', 'flac': '.flac', 'wav': '.wav'}
    if _ff:
        _mp3 = out_path if out_path.lower().endswith('.mp3') else os.path.splitext(out_path)[0] + '.mp3'
        try:
            import subprocess as _sp
            _sp.run([_ff, '-y', '-i', out_path, '-codec:a', 'libmp3lame', '-b:a', '192k', _mp3],
                    stdout=_sp.DEVNULL, stderr=_sp.DEVNULL, check=True)
            os.remove(out_path)
            out_path = _mp3
            print(json.dumps({'format_fix': True, 'from': _fmt, 'to': 'mp3'}, ensure_ascii=False))
        except Exception as _e:
            _alt = os.path.splitext(out_path)[0] + _ext_map.get(_fmt, '.dat')
            shutil.move(out_path, _alt)
            out_path = _alt
            print(json.dumps({'format_fix': False, 'reason': str(_e), 'path': out_path}, ensure_ascii=False))
    else:
        _alt = os.path.splitext(out_path)[0] + _ext_map.get(_fmt, '.dat')
        shutil.move(out_path, _alt)
        out_path = _alt
        print(json.dumps({'format_fix': False, 'reason': 'ffmpeg not found', 'path': out_path}, ensure_ascii=False))
