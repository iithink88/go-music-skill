...
# ---- backend acquisition: bundled offline copy first, then download ----
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BUNDLED="$SCRIPT_DIR/../backend/go-music-api"

if [ -f "$BIN" ] && file "$BIN" 2>/dev/null | grep -Eq 'ELF|Mach-O'; then
 echo "[music] backend already present"
elif [ -f "$BUNDLED" ] && file "$BUNDLED" 2>/dev/null | grep -Eq 'ELF|Mach-O'; then
 echo "[music] using bundled offline backend (no internet required): $BUNDLED"
 cp "$BUNDLED" "$BIN"
 chmod +x "$BIN"
 echo "[music] installed from bundle: $BIN"
else
...
# NOTE: the go-music-api binary HARDCODES port 8080 at runtime, so a dynamic
# free port would be ignored by the backend and break health checks. Always use
# 8080; if it is occupied by another process, refuse instead of pretending a
# different port works.
PORT=8080
if port_is_busy "$PORT"; then
  echo "[music] port $PORT is already in use by another process."
  echo "[music] the backend binary hardcodes this port; free it (kill the other process) and retry."
  exit 1
fi
