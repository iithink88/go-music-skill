...
- If the backend always binds to a fixed port in practice, simplify the port logic instead of pretending dynamic ports work. **Known fact (2026-08-04): `go-music-api` hardcodes port 8080.** The bundled `install.sh` / `install.ps1` now use 8080 explicitly and refuse to start if 8080 is occupied by another process. Always read the port from `~/.openclaw/music/port` (or `%USERPROFILE%\.openclaw\music\port`), defaulting to `8080`.
- The stream endpoint may return **M4A/AAC** (e.g. kuwo) even when you request `.mp3`. Detect the real container by magic bytes and transcode to MP3 with ffmpeg before embedding metadata; `embed_metadata.py` relies on mutagen ID3 and only works on real MP3. The bundled `play.sh` / `play.ps1` already do this.
